...
const ListItem = ({ title, company, location, details, startDate, endDate }) => (
    <div className="list-item">
        <div>
            <h3>
                {title} {company ? `| ${company} | ${location}` : location ? `| ${location}` : ''}
            </h3>
...
const EducationItem = ({ title, company, location, details, startDate, endDate, relevantCoursework, leadershipAndActivities, gpa }) => (
    <div className="list-item">
        <div>
            <h3>
                {title} {company ? `| ${company} | ${location}` : location ? `| ${location}` : ''}
        ...
        <InfoRow label="Leadership and Activities" content={leadershipAndActivities} />
    </div>
);
        ...
        {subBullets.length > 0 && (
            <ul>
                {subBullets.map((subText, index) => (
                    <li className="subBullet" key={index}>{subText}</li>
                ))}
            </ul>
        )}
    </ul>
            ...
            {show && (
                <div className="resume-content">
                            ...
                            details={
                                [
                                    ...
                                    <Bullet 
                                        ...
                                        subBullets={["Integrated end-to-end password validation displaying errors to users and disabling the 'register' button"]} 
                                    />,
                                    <Bullet 
                                        point="Launched a multi-layered system heuristic for tab naming conventions enhancing 80% of multi-query searches"
                                        subBullets={["Added manual title editing and customizable tab borders via a drop-down color menu"]}
                                    />,
                    ...
                    <Section title="Technical Knowledge">
                        <p>Frameworks: React, Spring Boot, Google Cloud, Azure, Node.js, Django, ASP.NET, Playwright, TensorFlow</p>
                        <p>Languages: JavaScript, TypeScript, HTML/CSS, Python, Java, C, C#, C++, Assembly, Lean, Racket, SQL</p>
                    </Section>

                    <Section title="Interests">
                        <p>Freelance abstract artist, mathematics, music composition, programming, piano, reading, skiing, traveling, yoga, weight training</p>
                    </Section>
                </div>
            )}
        </div>
